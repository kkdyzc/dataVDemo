  ...
  if (userStore.content2List.length > 0) {
    ...
    userStore.content2List[2].forEach((item: any) => {
      ...
      if (item['任务名称']?.length > 0)
        config.data.push([item['任务名称'], item?.url ? `<span class="urlColor">${item.url}</span>` : `<span class="urlColor">/data/工业/${item['任务名称']}</span>`, item?.ip || `172.16.0.25${Math.floor(Math.random() * (6 + 1))}`, item?.type || `/data/工业/${item['任务名称']}`])
    })
  }
